...
function ClassicCasinoSlider({ handlePlayPlinko }: { handlePlayPlinko: () => void }) {
  const classicGames = [
    { id: 1, title: "Aviator", icon: "✈️", description: "Watch the plane fly and cash out before it crashes" },
    { id: 2, title: "Coin Flip", icon: "🪙", description: "Simple heads or tails betting" },
    { id: 3, title: "Dice Roll", icon: "🎲", description: "Roll the dice and predict combinations" },
    { id: 4, title: "Scratch Cards", icon: "🎫", description: "Instant win scratch cards" },
    { id: 5, title: "Roulette", icon: "🎡", description: "Classic casino roulette wheel" },
    { id: 6, title: "Blackjack", icon: "🃏", description: "Beat the dealer with 21" }
  ];
      ...
      <div className="overflow-x-auto">
        <div className="flex space-x-4 pb-4" style={{minWidth: 'max-content'}}>
          {classicGames.map((game) => (
            <div key={game.id} className="bg-yellow-600 p-6 rounded-lg text-center min-w-[200px] hover:bg-yellow-500 transition-colors cursor-pointer">
              <div className="text-5xl mb-3">{game.icon}</div>
              <h3 className="font-bold text-white text-lg mb-2">{game.title}</h3>
              <p className="text-yellow-100 text-sm mb-3">{game.description}</p>
...
// New Games Slider
function NewGamesSlider({ handlePlayPlinko }: { handlePlayPlinko: () => void }) {
  const newGames = [
    { 
      id: 1, 
      title: "PLINKO", 
    ...
    { 
      ...
      icon: "🌈", 
      description: "Predict colors and multiply your winnings!",
      status: "COMING SOON",
      color: "bg-purple-600",
    ...
    { 
      ...
      title: "CRASH GAME", 
      icon: "🚀", 
      description: "Watch the multiplier grow and cash out!",
      status: "COMING SOON",
      color: "bg-blue-600",
  ...
  const handleGameClick = (game: any) => {
    if (game.isPlayable && game.title === "PLINKO") {
      handlePlayPlinko();
    }
  };
      ...
      <div className="overflow-x-auto">
        <div className="flex space-x-4 pb-4" style={{minWidth: 'max-content'}}>
          {newGames.map((game) => (
            <div 
              key={game.id} 
              className={`${game.color} p-8 rounded-lg text-center min-w-[250px] hover:scale-105 transition-all cursor-pointer border-2 border-white`}
              onClick={() => handleGameClick(game)}
            >
              <div className="text-6xl mb-4">{game.icon}</div>
              <h3 className="font-bold text-white text-xl mb-3">{game.title}</h3>
              <p className="text-white text-sm mb-4 opacity-90">{game.description}</p>
              <button className={`bg-white ${game.textColor} px-6 py-3 rounded-lg font-bold text-lg hover:opacity-90 transition-colors`}>
                {game.status}
              </button>
            </div>
          ))}
  ...
  return (
    <div className="min-h-screen bg-casino-primary">
      ...
      <div className="container mx-auto px-4 py-6 space-y-6">
        {/* Classic Casino Games Carousel */}
        <ClassicCasinoSlider handlePlayPlinko={handlePlayPlinko} />

        {/* New Games Carousel */}
        <NewGamesSlider handlePlayPlinko={handlePlayPlinko} />
